...
public class CalculadorP {
	...
	/**
	   ...
	   */
	public CalculadorP(double dX,int iDof, int iNum_seg){
		this.dX = dX;
		this.iDof = iDof;
	...
	public double calculaTDist(double dXi){
		...
		dT = Math.pow((1 + (dXi*dXi)/iDof),-1*((iDof+1)/2.0));
    double gammaDenominador;
    double gammaNumerador;
		//Se multiplica por el lado izquierdo
    if(iDof%2 == 0){
      gammaDenominador = calculaGammaEntera(iDof/2);
    }
    else{
      gammaDenominador = calculaGammaFraccion(iDof/2.0);
    }
    if((iDof+1)%2==0){
      gammaNumerador = calculaGammaEntera((iDof+1)/2);
    }
    else{
      gammaNumerador = calculaGammaFraccion((iDof+1)/2.0);
    }
		dT *= gammaNumerador/((Math.pow(iDof*Math.PI,0.5))*gammaDenominador);
